    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative flex justify-center items-center py-8 px-4">
        <div className="w-full max-w-xl rounded-2xl bg-gradient-to-r from-main to-sky-400 shadow-xl flex items-center gap-4 p-6 animate-fade-in-up">
          ...
          <div>
            <h1 className="text-white text-2xl md:text-3xl font-bold mb-1">{t('post_ad_form.post-your-ad-today')}</h1>
            <p className="text-white/90 text-sm md:text-base">
              {t('post_ad_form.banner-encourage-message')}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center items-center py-4">
        ...
        >
          ...
          <div>
            <label className="block mb-1 font-semibold text-main">{t('post_ad_form.select-category')} <span className="text-red-500">*</span></label>
          ...
          {data.category_id && (
            <div>
              <label className="block mb-1 font-semibold text-main">{t('post_ad_form.select-subcategory')} <span className="text-red-500">*</span></label>
              <select
              ...
              >
                <option value="">{t('post_ad_form.select-subcategory')}</option>
                {filteredSubcategories.map((sub: any) => (
                  <option key={sub.id} value={sub.id}>{i18n.language === "ar" ? sub.title_ar : sub.title_en}</option>
                ))}
              </select>
              {errors.subcategory_id && (
                <div className="text-red-500 text-sm mt-1">{errors.subcategory_id}</div>
              )}
            </div>
          )}
          ...
          {/* Images Upload */}
          <div>
            <label className="block mb-1 font-semibold text-main">{t('post_ad_form.images')}</label>
            <div
            ...
            >
              <input
                ...
                className="hidden"
              />
              <span className="text-main font-medium group-hover:underline">{t('post_ad_form.drag-images')}</span>
              <span className="text-xs text-gray-400 mt-1">{t('post_ad_form.select-many')}</span>
            </div>
          ...
          >
            {processing ? t('post_ad_form.publishing') : t('post_ad_form.publish')}
          </button>
        </form>
      </div>
      <Custommodal modalname={modalRef} message={t('post_ad_form.ad-published-successfully')} />
      <Footer />
      <BottomNavbar />
    </div>
  )
